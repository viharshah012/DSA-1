...
public class BinaryTree<T> {
	...
	private String getInOrder(TreeNode<T> curNode) {
		...
		order = order + getInOrder(curNode.left);
		order = order + (curNode.data.toString() + " ");
		order = order + getInOrder(curNode.right);
		return order;
	}
	...
	private String getPreOrder(TreeNode<T> curNode) {
		...
		order = order + (curNode.data.toString() + " ");
		order = order + getPreOrder(curNode.left);
		order = order + getPreOrder(curNode.right);
		return order;
	}
	...
	private String getPostOrder(TreeNode<T> curNode) {
		...
		order = order + getPostOrder(curNode.left);
		order = order + getPostOrder(curNode.right);
		order = order + (curNode.data.toString() + " ");
		return order;
	}
